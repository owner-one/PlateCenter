	...
	b.prototype = {
				...
				a.each(g.data, function(h, j) {
					...
					if (j.type === "folder") {
						...
						if (j.type === "item") {
							...
							i.data(j);
							if ("additionalParameters" in j && "item-selected" in j.additionalParameters && j.additionalParameters[
									"item-selected"] == true) {
		...
		selectItem: function(e) {
			...
			if (this.options.multiSelect) {
				...
				if (g[0] !== d[0]) {
					g.removeClass("tree-selected").find("i").removeClass(this.options["selected-icon"]).addClass(this.options[
						"unselected-icon"]);
					f.push(d.data())
				}
			}
		...
		selectFolder: function(e) {
			...
			if (d.find("." + this.options["close-icon"]).length) {
				...
				f.find("." + this.options["close-icon"] + ":eq(0)").removeClass(this.options["close-icon"]).addClass(this.options[
					"open-icon"]);
				...
				f.find("." + this.options["open-icon"] + ":eq(0)").removeClass(this.options["open-icon"]).addClass(this.options[
					"close-icon"]);
